...
def _image_args(args_parser):  # pragma: no cover
    ...
    args.add_argument('-W', '--no-webp', action='store_true',
...
def _debug_args(args_parser):  # pragma: no cover
    ...
    args.add_argument('-l', '--simulate', action='store_true',
                      help='Simulate running %(prog)s, where: 1) do not download files and, 2) do not write anything on disk.')

    args.add_argument('-i', '--show-current-chapter-info', action='store_true',
    ...
    args.add_argument('-b', '--debug', action='store_true', help='Debug %(prog)s.')
...
def _downloading_args(args_parser):  # pragma: no cover
    ...
    args.add_argument('-m', '--max-volumes', metavar='COUNT', type=int, default=0,
    ...
    args.add_argument('-a', '--user-agent', type=str,
                      help='Set an user-agent. Don\'t work from protected sites.')
    args.add_argument('-x', '--proxy', type=str, help='Set a http proxy.')
    args.add_argument('-e', '--reverse-downloading', action='store_true',
    ...
    args.add_argument('-w', '--rewrite-exists-archives', action='store_true',
    ...
    args.add_argument('-g', '--with-manga-name', action='store_true',
    ...
    args.add_argument('-c', '--min-free-space', metavar='MB', type=int, default=100,
...
def _reader_args(args_parser):  # pragma: no cover
    ...
    args.add_argument('-z', '--cbz', action='store_true',
                      help='Make `*.cbz` archives (for reader).')

    args.add_argument('-r', '--rename-pages', action='store_true',
...
def get_cli_arguments() -> ArgumentParser:  # pragma: no cover
    ...
    args.add_argument('-v','--version', action='version', version=__version__, help='Show %(prog)s\'s version number and exit.')
