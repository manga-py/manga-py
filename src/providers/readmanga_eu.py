...
class ReadMangaEu(Provider):
    ...
    def get_chapter_index(self) -> str:
        idx = self.re_search('/manga/\d+/[^/]+/([^/]+)', self.get_current_chapter()).group(1).split('.')
        return '{}-{}'.format(
            idx[0],
    ...
    def get_main_content(self):
        name = self.re_search('/(manga/\d+/[^/]+)', self.get_url()).group(1)
        return self.http_get('{}/{}'.format(self.get_domain(), name))

    def get_manga_name(self) -> str:
        return self.re_search('/manga/\d+/([^/]+)', self.get_url()).group(1)
