...
class DesuMe(Provider):
    ...
    def get_archive_name(self) -> str:
        return self.re.search('/(vol\\d+/ch\\d+)', self.get_current_chapter()).group(1).replace('/', '_')

    def get_chapter_index(self) -> str:
        result = self.re.search('/vol(\\d+)/ch(\\d+)', self.get_current_chapter()).groups()
    ...
    def get_chapters(self):
        parser = self.document_fromstring(self.storage_main_content(), '#animeView ul h4 > a.tips')
